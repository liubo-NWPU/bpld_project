package com.gis.manager.annotation.paramchecker;
...
public class CheckParamAspect {
    ...
    @Around(value="@annotation(Check)") // 这里要换成自定义注解名称
    public Object check(ProceedingJoinPoint point) throws Throwable {
        Object obj;
        // 参数校验
        List<String> msg = doCheck(point);
        if (msg.size() != 0) {
...
//            throw new IllegalArgumentException(msg);
    ...
    Map<String,Object> getNameAndValue(ProceedingJoinPoint point){
     ...
     * @return 错误信息
     */
    private List<String> doCheck(ProceedingJoinPoint point) {
        List<String> msgTotal = new ArrayList<>();
        // 获取方法参数值
        ...
        for (String field : fields) {
            ...
            // 获取字段的值
            Object result = params.get(info.field);
            if (result == null){
                ...
                for (int i = 0; i<paramsAnnotation.length; i++){
                    ...
                    for (Annotation pann : annotations){
                        if (pann.annotationType().equals(RequestBody.class)){
                            try{
                                result = ReflectionUtil.invokeGetter(argument, info.field);
            ...
            // 执行校验规则
            Boolean isValid = info.optEnum.fun.apply(result, info.operatorNum);
            if (!isValid){
                msgTotal.add(info.innerMsg);
            }
        }
        return msgTotal;
    }
